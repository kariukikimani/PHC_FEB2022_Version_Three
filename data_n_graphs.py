...
def gen_main_graph(facility, sel_graph):
    ...
    # Two column graph -> bar-bar and bar-line
        ...
        if graph_type == 'bar-bar':
            ...
            main_grf.add_trace(
                go.Scatter(x=fac_filter['Year'], y=fac_filter[y2], name=y2, marker_color='#ff9900', marker_size=5,
                           marker_line_width=5)
            )
...
def kpi_graphs(sel_aim, facility, visibility):
    ...
    kpi_df = pd.DataFrame(
        columns=['KPI', 'Start Year', 'End Year', 'Start Year Value', 'End Year Value', 'Change %', 'Status'])
